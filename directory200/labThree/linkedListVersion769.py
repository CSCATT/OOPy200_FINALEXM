...
from directory200.modulesL5Final.structureDriver import*
from directory200.modulesL5Final.observer import *
...
class LinkedList(IStructureDriver, Observer):
	...
	def __to_dict (self):
		...
		for i in range(self.len):
			res[i] = node.data
			# res.append({'data': node.data, 'prev': node.prev, 'nxt': node.next_})
			node = node.next_
		return res
